...
# Copy WCNSS_cfg.dat file from current folder to target out directory.
ifeq ($(WLAN_PROPRIETARY),0)

$(shell mkdir -p $(TARGET_OUT_ETC)/firmware/wlan/prima)
$(shell rm -f $(TARGET_OUT_ETC)/firmware/wlan/prima/WCNSS_cfg.dat)
$(shell cp $(LOCAL_PATH)/$(LOCAL_PATH)/WCNSS_cfg.dat $(TARGET_OUT_ETC)/firmware/wlan/prima)
...
LOCAL_SRC_FILES    := $(LOCAL_PATH)/$(LOCAL_MODULE)
...
VERSION=$(shell grep -w "VERSION =" $(TOP)/kernel/xiaomi/msm8953/Makefile | sed 's/^VERSION = //' )
PATCHLEVEL=$(shell grep -w "PATCHLEVEL =" $(TOP)/kernel/xiaomi/msm8953/Makefile | sed 's/^PATCHLEVEL = //' )
...
LOCAL_MODULE_PATH         := $(TARGET_COPY_OUT_VENDOR)/lib/modules/$(WLAN_CHIPSET)
else
...
ifeq ($(PRODUCT_VENDOR_MOVE_ENABLED), true)
$(shell mkdir -p $(TARGET_COPY_OUT_VENDOR)/lib/modules; \
        ln -sf /$(TARGET_COPY_OUT_VENDOR)/lib/modules/$(WLAN_CHIPSET)/$(WLAN_CHIPSET)_wlan.ko \
               $(TARGET_COPY_OUT_VENDOR)/lib/modules/wlan.ko)
else
